...
const ProductCard3List = () => {
    ...
    const wishlist = useSelector((state:any)=>state.new.search_results)
    let shop = useSelector((state:any)=>state.new.shop)
    const router = useRouter()
    const {page} = router.query
    useEffect(() => {
        let currency_text = `&currency=${Cookies.get("currency_id")}`
        axios.get(`${BASE_URL}/flowers/search/ru?page=${page}&shop_id=${shop?.data?.id}${currency_text}`)
            .then(response=>{
    ...
    return (
        <div>
                ...
                {wishlist?.datas?.data && wishlist?.datas?.data?.length !== 0 ? <>
...
export default ProductCard3List;
