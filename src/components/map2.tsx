...
const Map20 = (props) =>  {
  const coordinate = props.coordinate === undefined ? [41.316441,69.294861] : props.coordinate
  ...
  const handlemap = (map) =>{
     const y =  map.geocode(coordinate)
      console.log(y)
  }
  // useEffect(() => {
       ...
       query={{
        ...
        <Map
            onClick={(e)=>{
                ...
                dispatch(getcoordinates(e._sourceEvent.originalEvent.coords))
            }
            }//.toLocaleString(coordinate)
            //originalEvent.map._yandexStateGeoObjects._yandexState._data.map
            width="100%"
          modules={[
        ...
        >
            {coordinate !== undefined ? <Placemark onClick={(r)=>console.log("Placemark",r)} geometry={coordinate} /> : ""}
          <SearchControl
            instanceRef={(ref) => {
