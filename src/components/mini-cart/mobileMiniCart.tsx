...
const MiniCart2: React.FC<MiniCartProps> = ({ toggleSidenav ,mobile}) => {
  ...
  let [currency_name,setcurrency_name] = useState([{name:""}])
  let current_currencies;
  let currencies = useSelector((state:any)=>state.token.currencies);
  useEffect(() => {
    current_currencies = Cookies.get("currency_id");
    setcurrency_name(currencies.filter(currency=>currency.id === parseInt(current_currencies) ? currency :""))
      ...
      <StyledMiniCart style={mobile&& cartList?.length ? {height:"fit-content"} : {height:"70vh"}} >
                  ...
                  <div className="cart-item">
                    ...
                    <div className="product-details">
                      ...
                      <Typography
                          ...
                          mt="4px"
                      >
                        {(item.qty * Number.parseFloat(item.price.toString().replace(/ /g,"")))}{currency_name.length !== 0 ? currency_name[0].name : currencies.filter(currency=>currency.code === "UZS" ? currency : "" )[0].name}
                      </Typography>
                    </div>
        ...
        {!!cartList.length && (
              ...
              <Link href="/cart">
                ...
                >
                  <Typography fontWeight={600} >
                    <FormattedMessage id="Checkout Now" defaultMessage="Hoziroq sotib olish" /> ({getTotalPrice().toFixed(2)} {currency_name.length !== 0 ? currency_name[0].name : currencies.filter(currency=>currency.code === "UZS" ? currency : "" )[0].name})
                  </Typography>
                </Button>
              </Link>
